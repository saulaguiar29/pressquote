...
import { Package, Plus, Edit2, Trash2, AlertCircle, AlertTriangle, RotateCcw } from 'lucide-react';
...
const ARCHIVED_KEY = 'pressquote_archived_materials';
...
export default function MaterialsPage() {
  ...
  useEffect(() => {
    api.getMaterials().then(data => {
      // Filter out any that are already archived so they don't show in both lists
      const archivedIds = new Set((JSON.parse(localStorage.getItem(ARCHIVED_KEY) || '[]')).map(m => m.id));
      setMaterials(data.filter(m => !archivedIds.has(m.id)));
    }).finally(() => setLoading(false));
  }, []);

  const saveArchived = (next) => {
    setArchived(next);
  ...
  const handleArchive = (m) => {
    ...
    saveArchived([...archived, m]);
  };

  const handleRestore = (m) => {
    saveArchived(archived.filter(x => x.id !== m.id));
    setMaterials(prev => [...prev, m].sort((a, b) => a.name.localeCompare(b.name)));
  };

  const handlePermanentDelete = async (m) => {
    try {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
                  ...
                  <tr key={m.id} className="table-row">
                    ...
                    <td className="table-cell text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        <button onClick={() => handleArchive(m)} className="btn-ghost p-1.5 text-xs text-red-400 hover:text-red-300"><Trash2 size={12} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {archived.length > 0 && (
        <div className="mt-8">
          <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-3">Previously Used</h2>
          <div className="card overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                ...
                <tbody>
                  {archived.map(m => (
                    <tr key={m.id} className="table-row opacity-60">
                      <td className="table-cell font-medium text-gray-900">{m.name}</td>
                      <td className="table-cell text-gray-900 hidden md:table-cell">{m.unit_type}</td>
                      <td className="table-cell text-right font-mono text-sm text-gray-900">{fmt(m.unit_cost)}</td>
                      <td className="table-cell text-gray-900 hidden lg:table-cell">{m.supplier || '—'}</td>
                      <td className="table-cell text-right">
                        <div className="flex justify-end gap-1">
                          <button onClick={() => handleRestore(m)} className="btn-ghost p-1.5 text-xs text-emerald-400 hover:text-emerald-300" title="Restore"><RotateCcw size={12} /></button>
                          <button onClick={() => handlePermanentDelete(m)} className="btn-ghost p-1.5 text-xs text-red-400 hover:text-red-300" title="Delete permanently"><Trash2 size={12} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
