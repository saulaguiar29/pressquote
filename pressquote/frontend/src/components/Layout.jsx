...
export default function Layout() {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
        ...
        {navItems.map((item, i) => {
          if (item.divider) return (
            <div key={i} className="px-2 pt-4 pb-1">
              <span className="text-xs font-semibold text-forest-400 uppercase tracking-widest">{item.label}</span>
            </div>
          );
      ...
      {/* User footer */}
      <div className="px-3 py-3 border-t border-forest-800">
        <div className="flex items-center gap-3 px-2 py-2 rounded-lg">
          <div className="w-7 h-7 rounded-full bg-forest-500/30 border border-forest-400/40 flex items-center justify-center text-forest-300 text-xs font-bold">
            {user?.name?.[0]?.toUpperCase()}
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-xs font-medium text-white truncate">{user?.name}</div>
            <div className="text-xs text-forest-300 capitalize">{user?.role}</div>
          </div>
          <button onClick={handleLogout} className="text-forest-400 hover:text-red-400 transition-colors p-1" title="Logout">
            <LogOut size={14} />
          </button>
        </div>
      </div>
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex flex-col w-56 bg-forest-900 border-r border-forest-800 shrink-0">
        <SidebarContent />
      </aside>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          <div className="w-56 bg-forest-900 border-r border-forest-800 flex flex-col">
            <SidebarContent />
          </div>
